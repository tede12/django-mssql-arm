...
"""
    ...
    def _alter_field(self, model, old_field, new_field, old_type, new_type,
        ...
        if old_field.column != new_field.column:
            ...
            for index_name in index_names:
                if (index_name.endswith('uniq')):
                    ...
                    self.execute(
                        self._db_table_delete_constraint_sql(self.sql_delete_index, model._meta.db_table, index_name))
        ...
        for old_rel, new_rel in rels_to_update:
            ...
            for index_name in index_names:
                self.execute(
                    self._db_table_delete_constraint_sql(self.sql_delete_index, old_rel.related_model._meta.db_table,
                                                         index_name))
            self.execute(
                self.sql_alter_column % {
    ...
    if django_version >= (4, 0):
        def _create_unique_sql(self, model, fields, name=None, condition=None, deferrable=None, include=None,
